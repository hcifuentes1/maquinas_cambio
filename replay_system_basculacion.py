...
# Definir las funciones de callback fuera del scope de register_replay_callbacks_basculacion





# Definir todas las funciones de callback fuera de register_replay_callbacks_basculacion

def machine_options_callback(selected_date, replay_system):
    """Callback para obtener opciones de máquinas por fecha"""
...
def basculacion_options_callback(selected_date, selected_machine, replay_system):
    """Callback para obtener opciones de eventos de basculación"""
...
# En replay_system_basculacion.py

# Paso 1: Define todas las funciones de callback FUERA de register_replay_callbacks_basculacion
...
def replay_status_callback_wrapper(n_play, n_pause, n_stop, n_intervals,
                                   selected_date, selected_machine, selected_basculacion,replay_system, speed):
   ...
   try:
       if trigger_id == 'replay-play-button' and selected_date and selected_machine and selected_basculacion:
           ...
           try:
               from hmi_dashboard import estado_maquinas, data_lock
               with data_lock:
           ...
           replay_system.current_replay['paused'] = not replay_system.current_replay['paused']
           ...
           if replay_system.current_replay.get('paused', False):
               current_idx = replay_system.current_replay['current_index']
               progress = (current_idx / replay_system.current_replay['total_frames']) * 100
               return [f"Pausado en posición {current_idx+1}/{replay_system.current_replay['total_frames']}", progress] + date_values
           
           # Avanzar reproducción según velocidad
           speed = replay_system.current_replay['speed']
           ...
           try:
               ...
               with data_lock:
                   if selected_machine in estado_maquinas:
                       ...
                       print(f"FORZADA actualización directa para {selected_machine}: Pos={estado['posicion']}, Prog={estado['ciclo_progreso']}")
           except Exception as e:
...
# Paso 2: Función para registrar callbacks SIN funciones anidadas
...
def register_replay_callbacks_basculacion(app, replay_system):
    ...
    # Registrar callbacks utilizando funciones lambda
    ...
    app.callback(
      ...
      replay_status_callback_wrapper(n_play, n_pause, n_stop, n_intervals,
